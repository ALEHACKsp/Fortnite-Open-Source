...
namespace rend {
	...
	HRESULT __stdcall hk_present(IDXGISwapChain* pSwapChain, UINT sync_interval, UINT flags)
	{
		...
		if (!device) {
			...
			if (m_pFont == NULL)
			{
				MessageBoxA(0, xorstr("couldn't find the font."), xorstr(""), MB_ICONERROR);
				exit(0);
			}
							...
							if (root)
							{
								...
								if (Util::WorldToScreen(width, height, &position.X)) {
									...
									if (playerName.c_str()) {
										...
										DrawNormalText(m_pFont, TextFormat(xorstr("%s %.0fm"), copy, dist), ImVec2(headPos.X, headPos.Y - 20), 12.0f, abccccc, true);
									}
								}
							}
						}
	...
	BOOLEAN Initialize()
	{
		...
		if (FAILED(D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, 0, 0, &featureLevel, 1, D3D11_SDK_VERSION, &sd, &swapChain, &device, nullptr, &context))) {
			MessageBox(0, xorstr(L"failed: dx11"), xorstr(L""), MB_ICONERROR);
			return FALSE;
		}
